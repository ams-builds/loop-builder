...
- **The fix is a scheduled re-check, not vigilance.** Manually remembering to sync two documents works until it doesn't. If the platform supports scheduled or recurring tasks, set one up to periodically compare the loop's checklist against its source and update the checklist if they've diverged.
